  ...
  useEffect(() => {
    if (mostrarDetallesRechazo && solicitud.operador_rechazo_id && !datosRechazo) {
      const cargarDatosOperador = async () => {
        ...
        try {
          const result = solicitud.operador_rechazo_id
            ? await fetchOperadorInfoAction(solicitud.operador_rechazo_id)
            : null;

          if (result?.success) {
            setDatosRechazo({
              nombreOperador: result.data.nombre,
              rolOperador: result.data.rol,
            });
          } else {
            setDatosRechazo({
  ...
  useEffect(() => {
    if (mostrarDetallesAprobacion && solicitud.operador_aprobacion_id && !datosAprobacion) {
      const cargarDatosAprobador = async () => {
        ...
        try {
          const result = solicitud.operador_aprobacion_id
            ? await fetchOperadorInfoAction(solicitud.operador_aprobacion_id)
            : null;
