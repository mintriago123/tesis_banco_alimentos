...
export interface EstadisticasBajas {
  total: { bajas: number; cantidad: number };
  por_motivo: {
...
export interface AlertasVencimientoResponse {
  success: boolean;
  configuracion: { dias_umbral: number; solo_vencidos?: boolean };
  estadisticas: {
    total: number;
    ...
    por_prioridad: { vencidos: number; alta: number; media: number; baja: number };
  };
