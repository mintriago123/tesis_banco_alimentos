...
import { aprobarSolicitudBodegaAction, rechazarSolicitudBodegaAction } from '@/modules/shared/bodegas/actions';
import type { BodegaSolicitud, BodegaSolicitudEstado, BodegaSolicitudFilters } from '@/modules/shared/bodegas/types';
...
const typeLabel = { ALTA: 'Alta', MODIFICACION: 'Modificación' } as const;
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-shadow hover:border-slate-300 hover:shadow-md">
      ...
      <div className="mt-5 flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
        <span className={`text-xs font-medium ${isPending ? 'text-amber-700' : 'text-slate-500'}`}>{isPending ? 'Requiere revisión' : 'Solicitud revisada'}</span>
...
export default function BodegaRequestsInbox() {
  const { filteredSolicitudes, solicitudes, filters, setFilters, isLoading, errorMessage, refetch } = useBodegaRequests();
  ...
  const counters = useMemo(
    () =>
      solicitudes.reduce<Partial<Record<BodegaSolicitudEstado, number>>>((accumulator, solicitud) => {
  ...
  const handleApprove = async () => {
    ...
    setProcessingId(id);
    setMessage(null);
    const result = await aprobarSolicitudBodegaAction(id);
    if (result.success) {
      finishReview();
  ...
  const handleReject = async () => {
    ...
    setProcessingId(id);
    setMessage(null);
    const result = await rechazarSolicitudBodegaAction(id, motivoRechazo);
    if (result.success) {
      finishReview();
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5" aria-label="Filtros de solicitudes">
        <div className="grid gap-3 lg:grid-cols-[1fr_220px_auto]">
          <label className="relative block">
            ...
            <TextInput type="search" value={filters.search} onChange={(event) => updateFilters({ search: event.target.value })} placeholder="Buscar donante o bodega" className="pl-9" />
          </label>
          <label>
            <span className="sr-only">Filtrar por estado</span>
            <SelectInput value={filters.estado} onChange={(event) => updateFilters({ estado: event.target.value as BodegaSolicitudFilters['estado'] })}>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-600">
          ...
          {hasActiveFilters && (
            <Button type="button" variant="ghost" className="mt-3" onClick={() => setFilters({ estado: 'PENDIENTE', search: '' })}>
              Limpiar filtros
            </Button>
          )}
        </div>
