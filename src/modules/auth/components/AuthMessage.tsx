...
export const AuthMessage: React.FC<AuthMessageProps> = ({ mensaje, onClose }) => {
  ...
  return (
    <div className={`flex items-start gap-3 rounded-xl border p-4 ${estilosPorTipo[mensaje.tipo]}`} role={mensaje.tipo === 'error' ? 'alert' : 'status'}>
      ...
      {onClose && (
        <button
          type="button"
          onClick={onClose}
          className="flex-shrink-0 ml-auto -mx-1.5 -my-1.5 rounded-lg focus:ring-2 p-1.5 inline-flex h-8 w-8 items-center justify-center hover:bg-black/10"
          aria-label="Cerrar"
        >
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path
