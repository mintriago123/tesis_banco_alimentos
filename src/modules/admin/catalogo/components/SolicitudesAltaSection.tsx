...
import { rechazarSolicitudAltaAction, type SolicitudAltaAlimento } from '@/modules/catalogo-solicitudes';
import { aprobarSolicitudViaApi } from '@/modules/catalogo-solicitudes/client';
...
async function notificarRevisionSolicitud(solicitud: SolicitudAltaAlimento) {
  await sendNotification({ event: 'catalog_food_request_reviewed', entityId: solicitud.id });
}

const ReviewModal = ({ mode, solicitud, unidades, categories, onClose, onApprove, onReject }: ReviewModalProps) => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (mode === 'aprobar') {
      await onApprove({ nombre, categoria: categoriaFinal, unidadIds, unidadPrincipalId });
    } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
        <div className="flex items-start justify-between border-b border-slate-200 pb-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-800">{mode === 'aprobar' ? 'Aprobar solicitud' : 'Rechazar solicitud'}</h2>
            <p className="text-xs text-slate-500">
              {solicitud.nombre} · {solicitud.categoria}
            </p>
          </div>
          ...
          {mode === 'aprobar' ? (
            <>
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label htmlFor="reviewNombre" className="mb-1 block text-sm font-medium text-slate-600">
                    Nombre
                  </label>
                  <input
                    id="reviewNombre"
                ...
                <div>
                  <label htmlFor="reviewCategoria" className="mb-1 block text-sm font-medium text-slate-600">
                    Categoría
                  </label>
                  <select
                    id="reviewCategoria"
              ...
              {categoria === 'personalizada' && (
                <div>
                  <label htmlFor="reviewCategoriaPersonalizada" className="mb-1 block text-sm font-medium text-slate-600">
                    Nueva categoría
                  </label>
                  <input
                ...
                <div className="max-h-72 space-y-4 overflow-y-auto rounded-xl border border-slate-200 bg-slate-50 p-3">
                  {Object.entries(unidadesPorTipo).map(([tipoId, unidadesTipo]) => (
                    <div key={tipoId} className="space-y-2">
                      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{unidadesTipo[0]?.tipo_magnitud_nombre ?? `Tipo ${tipoId}`}</p>
                            ...
                            <div key={unidad.id} className="relative">
                              <button
                                ...
                                className={`w-full rounded-lg border px-3 py-2 text-left text-sm transition-colors ${
                                  selected ? 'border-indigo-400 bg-indigo-50 text-indigo-900' : 'border-slate-200 bg-white text-slate-700 hover:border-indigo-300'
                                }`}
                              >
                              ...
                              {selected && (
                                <button
                                  ...
                                  className={`absolute -right-1 -top-1 rounded-full px-2 py-0.5 text-xs font-semibold ${
                                    principal ? 'bg-emerald-600 text-white' : 'bg-slate-200 text-slate-600 hover:bg-emerald-100'
                                  }`}
                                >
            ...
            <div>
              <label htmlFor="comentarioRechazo" className="mb-1 block text-sm font-medium text-slate-600">
...
const SolicitudesAltaSection = ({ solicitudes, unidades, categories, loading, onRefresh, onCatalogRefresh, onSuccess, onError }: SolicitudesAltaSectionProps) => {
  ...
  const handleApprove = async (values: { nombre: string; categoria: string; unidadIds: number[]; unidadPrincipalId?: number }) => {
    ...
    const result = await aprobarSolicitudViaApi({ solicitudId: solicitud.id, ...values });
  ...
  const handleReject = async (comentarioAdmin: string) => {
    ...
    const result = await rechazarSolicitudAltaAction({ solicitudId: solicitud.id, comentarioAdmin });
  ...
  if (loading) {
    return <div className="rounded-2xl border border-slate-200 bg-white p-6 text-sm text-slate-500 shadow-sm">Cargando solicitudes de alta...</div>;
  }

  if (solicitudes.length === 0) {
    return <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center text-slate-500 shadow-sm">No hay solicitudes de alta de alimentos.</div>;
  }
  ...
  return (
    <div className="space-y-4">
              ...
              {solicitudes.map((solicitud) => (
                <tr key={solicitud.id} className="hover:bg-slate-50">
                  <td className="px-4 py-4">
                    ...
                    <div className="mt-1 text-xs text-slate-400">{solicitud.unidades.map((unidad) => unidad.unidad?.simbolo ?? unidad.unidad_id).join(', ')}</div>
                  </td>
                  <td className="px-4 py-4 text-sm text-slate-600">{solicitud.solicitante?.nombre || solicitud.solicitante?.email || 'Donante'}</td>
                  <td className="px-4 py-4">
                    <span className={`rounded-full border px-2 py-1 text-xs font-semibold capitalize ${estadoStyles[solicitud.estado]}`}>{solicitud.estado}</span>
                  </td>
                  <td className="px-4 py-4">
                    {solicitud.estado === 'pendiente' ? (
                      ...
                      <span className="text-xs text-slate-400">Revisada el {solicitud.fecha_revision ? new Date(solicitud.fecha_revision).toLocaleDateString('es-EC') : '-'}</span>
                    )}
                  </td>
                </tr>
              ))}
      ...
      {selected && <ReviewModal mode={modalMode} solicitud={selected} unidades={unidades} categories={categories} onClose={closeModal} onApprove={handleApprove} onReject={handleReject} />}
    </div>
  );
};
