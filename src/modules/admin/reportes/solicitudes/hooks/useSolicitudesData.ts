...
import { INITIAL_FILTERS, ESTADO_BADGE_STYLES, ESTADO_ICONS, ESTADO_LABELS, SYSTEM_MESSAGES } from '../constants';
import type { LoadingState, Solicitud, SolicitudEstado, SolicitudFilters, SolicitudEstadoFilter, SolicitudCounters } from '../types';
import { fetchSolicitudesAction } from '../actions';
...
const applyFilters = (solicitudes: Solicitud[], filters: SolicitudFilters) => {
  ...
  if (search.trim()) {
    ...
    result = result.filter((solicitud) => {
      ...
      return nombre.includes(term) || cedula.includes(term) || telefono.includes(term) || tipoAlimento.includes(term);
    });
  }

  if (!estados.todos) {
    const estadosActivos = (['pendiente', 'aprobada', 'rechazada', 'entregada'] as SolicitudEstado[]).filter((estado) => estados[estado]);
...
const buildCounters = (solicitudes: Solicitud[]): SolicitudCounters => {
  return solicitudes.reduce<SolicitudCounters>(
    (acc, solicitud) => {
      acc[solicitud.estado] += 1;
      acc.total += 1;
      return acc;
    },
    { pendiente: 0, aprobada: 0, rechazada: 0, entregada: 0, total: 0 },
  );
};

export const useSolicitudesData = (): UseSolicitudesDataResult => {
  ...
  const loadSolicitudes = useCallback(async () => {
    ...
    const result = await fetchSolicitudesAction();
  ...
  const filteredSolicitudes = useMemo(() => applyFilters(solicitudes, filters), [solicitudes, filters]);

  const counters = useMemo(() => buildCounters(solicitudes), [solicitudes]);
  ...
  const toggleEstadoFilter = useCallback((estado: keyof SolicitudEstadoFilter) => {
    setFilters((prev) => {
      if (estado === 'todos') {
        return { ...prev, estados: { ...INITIAL_FILTERS.estados } };
      }

      const updatedEstados = { ...prev.estados, todos: false, [estado]: !prev.estados[estado] };

      const algunoActivo = (['pendiente', 'aprobada', 'rechazada'] as SolicitudEstado[]).some((key) => updatedEstados[key]);

      if (!algunoActivo) {
        return { ...prev, estados: { ...INITIAL_FILTERS.estados } };
      }

      return { ...prev, estados: updatedEstados };
    });
  }, []);
