...
export const deliverSolicitud = async (params: DeliverSolicitudParams, deps: SolicitudUseCaseDeps): SolicitudActionResult => {
  ...
  if (!codigoEsperado) {
    return { success: false, error: 'La solicitud no tiene un comprobante aprobado asociado' };
  }

  if (!codigoIngresado) {
    return { success: false, error: 'Debes escanear o ingresar el código del comprobante para marcar la entrega' };
  }

  if (codigoIngresado !== codigoEsperado) {
    return { success: false, error: 'El código del comprobante no coincide con la solicitud' };
  }

  if (solicitud.estado !== 'aprobada') {
    return { success: false, error: 'Solo se pueden marcar como entregadas las solicitudes aprobadas' };
  }
  ...
  await deps.notificationService.notificarCambioEstado(solicitud, 'entregada', { comentarioAdmin });

  return {
    success: true,
    data: { success: true, message: 'Solicitud marcada como entregada exitosamente', warning: false },
  };
};
