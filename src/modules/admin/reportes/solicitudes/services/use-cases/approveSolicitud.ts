...
import { buildInventoryDiscountError, buildResultadoMensaje } from '../solicitudesResultMessages';
...
export const approveSolicitud = async (params: ApproveSolicitudParams, deps: SolicitudUseCaseDeps): SolicitudActionResult => {
  const { solicitud, comentarioAdmin, operadorId, depositoId, cantidadAprobada } = params;
  ...
  if (cantidadAprobada !== undefined) {
    const cantidadAprobadaResult = parseFiniteNumberValue(cantidadAprobada, { name: 'cantidadAprobada', min: 0, max: cantidadSolicitud.value });
    if (!cantidadAprobadaResult.success || cantidadAprobadaResult.value <= 0) {
      return {
        success: false,
        error: cantidadAprobadaResult.success ? 'cantidadAprobada debe ser mayor a 0.' : cantidadAprobadaResult.error,
      };
    }
  }

  const cantidadObjetivo = Math.max(0.01, Math.min(cantidadAprobada ?? solicitud.cantidad, solicitud.cantidad));
  const solicitudConCantidad = { ...solicitud, cantidad: cantidadObjetivo };

  const validacionStock = await deps.inventoryService.validarStockDisponiblePorDeposito(solicitud, depositoIdValidado, cantidadObjetivo);
  ...
  if (resultadoInventario.error || resultadoInventario.noStock || resultadoInventario.cantidadRestante > 0) {
    ...
    return {
      success: false,
      error: buildInventoryDiscountError(resultadoInventario.errorDetails, 'No fue posible descontar el inventario requerido para aprobar la solicitud'),
  ...
  const movimientoResult = await deps.movementService.registrarMovimientoSolicitud(solicitudConCantidad, resultadoInventario);
