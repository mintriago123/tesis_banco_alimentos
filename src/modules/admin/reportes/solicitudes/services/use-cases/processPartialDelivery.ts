...
import { historialDonaciones } from '@/db/schema';
import type { Solicitud } from '../../types';
import { rollbackSolicitudState, snapshotSolicitudState, updateSolicitudById } from './solicitudStatePersistence';
...
export const processPartialDelivery = async (params: ProcessPartialDeliveryParams, deps: SolicitudUseCaseDeps): SolicitudActionResult => {
  const { solicitud, cantidadDonar, porcentaje, comentario, operadorId, depositoId } = params;
  ...
  const cantidadSolicitud = parseFiniteNumberValue(solicitud.cantidad, { name: 'solicitud.cantidad', min: 0 });
  if (!cantidadSolicitud.success || cantidadSolicitud.value <= 0) {
    return { success: false, error: cantidadSolicitud.success ? 'solicitud.cantidad debe ser mayor a 0.' : cantidadSolicitud.error };
  }

  const cantidadDonarResult = parseFiniteNumberValue(cantidadDonar, { name: 'cantidadDonar', min: 0, max: cantidadSolicitud.value });
  if (!cantidadDonarResult.success || cantidadDonarResult.value <= 0) {
    return { success: false, error: cantidadDonarResult.success ? 'cantidadDonar debe ser mayor a 0.' : cantidadDonarResult.error };
  }
  ...
  const comentarioResult = parseOptionalTextValue(comentario, { name: 'comentario', maxLength: 500 });
  ...
  const validacionStock = await deps.inventoryService.validarStockDisponiblePorDeposito(solicitud, depositoValidado, cantidadDonarValidada);

  if (validacionStock.disponible === 0) {
    return { success: false, error: `La bodega seleccionada no tiene stock disponible para "${solicitud.tipo_alimento}".` };
  }

  if (cantidadDonarValidada <= 0 || cantidadDonarValidada > solicitud.cantidad) {
    return { success: false, error: `La cantidad a donar debe ser mayor a 0 y máximo ${solicitud.cantidad} ${solicitud.unidades?.simbolo ?? 'unidades'}` };
  }

  if (validacionStock.disponible < cantidadDonarValidada) {
    return { success: false, error: `Stock insuficiente para donar ${cantidadDonarValidada} unidades. Disponible: ${validacionStock.disponible} ${solicitud.unidades?.simbolo ?? 'unidades'}` };
  }
  ...
  if (resultadoInventario.error || resultadoInventario.noStock || resultadoInventario.cantidadRestante > 0) {
    ...
    return {
      success: false,
      error: buildInventoryDiscountError(resultadoInventario.errorDetails, 'No fue posible descontar el inventario requerido para registrar la entrega'),
  ...
  if (updateError) {
    ...
    return { success: false, error: 'No fue posible registrar la donación', errorDetails: updateError };
  }

  const movimientoResult = await deps.movementService.registrarMovimientoSolicitud(solicitudTemporal, resultadoInventario);

  if (!movimientoResult.success) {
    await deps.inventoryService.restaurarInventario(resultadoInventario.detalleEntregado);
    await rollbackSolicitudState(deps.db, solicitud.id, previousState);
    return { success: false, error: movimientoResult.error ?? 'No fue posible registrar el movimiento de inventario', errorDetails: movimientoResult.errorDetails };
  }

  try {
    await deps.db.insert(historialDonaciones).values({
      solicitudId: solicitudId.value,
      cantidadEntregada: String(cantidadDonarValidada),
      porcentajeEntregado: String(porcentajeValidado),
      cantidadSolicitada: String(solicitud.cantidad),
      operadorId: operadorIdValidado,
  ...
  return {
    success: true,
    data: { success: true, message: mensaje, warning: resultadoInventario.error || resultadoInventario.noStock },
  };
};
