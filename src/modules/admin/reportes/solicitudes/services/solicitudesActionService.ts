  ...
  const updateSolicitudEstado = async (
    ...
    try {
      ...
      if (!solicitudValidation.success) {
        return {
          success: false,
          error: solicitudValidation.error,
          errorDetails: solicitudValidation.errorDetails,
        };
      }
      ...
      if (nuevoEstado === 'aprobada' && solicitud.estado === 'pendiente') {
        return approveSolicitud({
          ...
          operadorId: operador.value,
          depositoId: deposito.value,
          cantidadAprobada: cantidad.value,
        }, deps);
      }
      ...
      if (nuevoEstado === 'rechazada') {
        return rejectSolicitud({
          solicitud,
          comentarioAdmin: comentario.value ?? undefined,
          motivoRechazo: motivo.value ?? undefined,
          operadorId: operador.value,
        }, deps);
      }
  ...
  const procesarDonacion = async (
    ...
    try {
      ...
      if (!cantidad.success || cantidad.value === undefined) {
        return {
          success: false,
          error: cantidad.success ? 'cantidadDonar es requerido.' : cantidad.error,
        };
      }
      ...
      if (!deposito.success || !deposito.value) {
        return {
          success: false,
          error: deposito.success ? 'depositoId es requerido.' : deposito.error,
        };
      }

      return processPartialDelivery({
        solicitud,
        cantidadDonar: cantidad.value,
        porcentaje: porcentajeResult.value,
        comentario: comentarioResult.value ?? undefined,
        operadorId: operador.value,
        depositoId: deposito.value,
      }, deps);
    } catch (err) {
...
function validateSolicitudForMutation(solicitud: Solicitud): ServiceResult<void> {
  ...
  if (!cantidad.success || cantidad.value <= 0) {
    return {
      success: false,
      error: cantidad.success ? 'solicitud.cantidad debe ser mayor a 0.' : cantidad.error,
    };
  }
...
function parseOptionalCantidad(
  value: number | undefined,
  max: number,
  name: string
): OptionalCantidadResult {
  if (value === undefined || value === null) {
    return { success: true, value: undefined };
  }

  const parsed = parseFiniteNumberValue(value, {
    name,
    min: 0,
    max,
  });
