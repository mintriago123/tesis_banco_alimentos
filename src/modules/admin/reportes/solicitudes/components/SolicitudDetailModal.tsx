...
import { fetchOperadorInfoAction } from '../actions';
  ...
  useEffect(() => {
    if (solicitud.estado === 'rechazada' && solicitud.operador_rechazo_id) {
      const cargarDatosOperador = async () => {
        ...
        try {
          const result = await fetchOperadorInfoAction(solicitud.operador_rechazo_id!);
          if (result.success) {
            setNombreOperador(result.data.nombre || 'No disponible');
            setRolOperador(result.data.rol || 'No disponible');
          }
        } catch (error) {
  ...
  useEffect(() => {
    if ((solicitud.estado === 'aprobada' || solicitud.estado === 'entregada') && solicitud.operador_aprobacion_id) {
      const cargarDatosAprobador = async () => {
        ...
        try {
          const result = await fetchOperadorInfoAction(solicitud.operador_aprobacion_id!);
          if (result.success) {
            setNombreAprobador(result.data.nombre || 'No disponible');
            setRolAprobador(result.data.rol || 'No disponible');
          }
        } catch (error) {
                      ...
                      <div>
                        ...
                        <span
                          className={`inline-block px-2 py-1 rounded text-xs font-semibold ml-1 ${
                            rolAprobador === 'ADMIN' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'
                          }`}
                        >
                          {rolAprobador === 'ADMIN' ? 'Administrador' : 'Operador'}
                        </span>
                      </div>
                      {solicitud.fecha_aprobacion && <p className="mt-1 text-xs text-gray-500">{formatDate(solicitud.fecha_aprobacion)}</p>}
                    </div>
                  )}
                </div>
              )}
                ...
                <div className="space-y-3">
                  {solicitud.motivo_rechazo && (
                    <div className="bg-white p-3 rounded border border-red-200">
                      ...
                      <p className="text-sm text-gray-700">{getMotivoRechazoLabel(solicitud.motivo_rechazo)}</p>
                    </div>
                  )}
                      ...
                      {cargandoOperador ? (
                        ...
                        <div className="text-sm text-gray-700">
                          ...
                          <div>
                            ...
                            <span
                              className={`inline-block px-2 py-1 rounded text-xs font-semibold ml-1 ${
                                rolOperador === 'ADMIN' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'
                              }`}
                            >
                        ...
                        <div className="mt-2">
                          {totalDisponible >= solicitud.cantidad ? (
                            ...
                            <div className="flex items-center text-red-600 text-sm">
                              ...
                              Stock insuficiente ({formatQuantity(totalDisponible)} de {formatQuantity(solicitud.cantidad)} disponibles)
                            </div>
                          )}
                        </div>
                      </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex space-x-3">
                  <button
                    ...
                    title={
                      !depositoSeleccionado
                        ? 'Selecciona una bodega para aprobar'
                        : !cantidadAprobadaValida
                          ? 'La unidad seleccionada requiere una cantidad entera'
                          : cantidadAprobar > maxAprobable
                            ? 'La cantidad supera el stock disponible en la bodega seleccionada'
                            : ''
                    }
                  >
