...
const hashDocument = (documentValue: string): string =>
  createHash('sha256').update(documentValue).digest('hex');
...
export function resolveServerServiceUrl(
  envKey: 'SERVICIO_CONSULTAS_DINARAP' | 'SERVICIO_CONSULTAS_RUC'
): ServiceUrlResult {
  ...
  if (!rawUrl) {
    return {
      success: false,
      response: configurationError('Servicio de consultas no configurado.'),
    };
  }
  ...
  try {
    ...
    return {
      success: false,
      response: configurationError('URL del servicio de consultas inválida.'),
    };
  }
...
export async function enforceDocumentLookupRateLimit(
  ...
  const { data, error } = await adminSupabase.rpc('consume_document_lookup_quota', {
    p_endpoint: endpoint,
    p_limit: DOCUMENT_LOOKUP_RATE_LIMIT.maxRequests,
    p_user_id: input.userId,
    p_window_seconds: DOCUMENT_LOOKUP_RATE_LIMIT.windowSeconds,
  ...
  if (error) {
    ...
    return {
      allowed: false,
      response: configurationError('No fue posible validar el límite de consultas.'),
    };
  }

  const quota = Array.isArray(data) ? (data[0] as ConsumeQuotaRow | undefined) : undefined;

  if (typeof quota?.allowed !== 'boolean') {
    return {
      allowed: false,
      response: configurationError('Respuesta inválida del limitador de consultas.'),
    };
  }

  const auditStatus = quota.allowed ? 'allowed' : 'blocked_rate_limit';
  const { error: auditError } = await adminSupabase
    .from('api_document_lookup_audit_log')
    .insert({
  ...
  if (auditError) {
    ...
    return {
      allowed: false,
      response: configurationError('No fue posible registrar la auditoría de la consulta.'),
    };
  }
