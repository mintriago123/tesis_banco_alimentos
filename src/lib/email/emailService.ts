...
export async function sendEmail(options: EmailOptions): Promise<void> {
  ...
  if (config.suppressSend) {
    ...
    if (isDevelopment) {
      console.log('[email] Envío de correo suprimido:', { to: options.to, subject: options.subject });
    }
    return;
  }
  ...
  if (config.logOnly) {
    ...
    if (isDevelopment) {
      console.log('[email] Envío de correo (modo log):', { to: options.to, subject: options.subject });
    }
  }
