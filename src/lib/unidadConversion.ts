...
export type UnitQuantityValidation = { valid: true } | { valid: false; error: string };
...
export const validarCantidadParaUnidad = (
  ...
  if (unidad.es_discreta && unidad.permite_fraccion === false && !Number.isInteger(cantidad)) {
    return {
      valid: false,
...
const isResolution = (value: unknown): value is ConversionResolution => {
  ...
  return value.reason === 'no_conversion' || value.reason === 'incompatible_magnitude' || value.reason === 'inactive_unit';
};

const noConversion = (): ConversionResolution => ({ convertible: false, reason: 'no_conversion' });
...
export const resolverConversionLocal = (
  ...
  if (unidadOrigenId === unidadDestinoId) {
    return { convertible: true, factor: 1, origenId: unidadOrigenId, destinoId: unidadDestinoId, source: 'same_unit' };
  }

  const conversion = conversiones.find(
    (item) => item.unidad_origen_id === unidadOrigenId && item.unidad_destino_id === unidadDestinoId,
  );

  if (conversion && conversion.activo !== false && isFinitePositive(conversion.factor_conversion)) {
    return { convertible: true, factor: conversion.factor_conversion, origenId: unidadOrigenId, destinoId: unidadDestinoId, source: 'database' };
  }

  const inverse = conversiones.find(
    (item) => item.unidad_origen_id === unidadDestinoId && item.unidad_destino_id === unidadOrigenId,
  );

  if (inverse && inverse.activo !== false && isFinitePositive(inverse.factor_conversion)) {
    return { convertible: true, factor: 1 / inverse.factor_conversion, origenId: unidadOrigenId, destinoId: unidadDestinoId, source: 'database' };
  }
...
/**
 * Resuelve definitivamente usando la función `resolver_conversion` de Postgres
 * (STABLE, SECURITY INVOKER — corre bajo el contexto RLS del caller, por eso
 * recibe una conexión ya envuelta en `withRlsContext`, nunca `dbAdmin`).
 */
export const resolverConversion = async (
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- accepts a Drizzle db/tx typed against any schema (Db, Tx, dbAdmin, ...)
  ...
  try {
    const [row] = await tx.execute<{ resolver_conversion: unknown }>(
      sql`select resolver_conversion(${unidadOrigenId}, ${unidadDestinoId}) as resolver_conversion`,
    );

    const data = row?.resolver_conversion;
 ...
 */
export const aplicarConversion = (cantidad: number, resolution: ConversionResolution): ConversionCalculation => {
  ...
  return { success: true, cantidad: resultado, factor: resolution.factor, resolution };
};
...
export const formatearNumero = (num: number, decimales = 2): string => {
  ...
  if (redondeado >= 10000) {
    return redondeado.toLocaleString('es-EC', { maximumFractionDigits: decimales });
  }
