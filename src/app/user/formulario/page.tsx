...
import { useDatosBasicosUsuario } from '@/modules/shared/hooks/useProfileUpdate';
import {
  useAlimentos,
  useUnidades,
  useUbicacion,
...
export default function FormularioSolicitante() {
  const { userData } = useDatosBasicosUsuario();
  const { alimentosFiltrados, categorias, busqueda, filtroCategoria, setBusqueda, setFiltroCategoria, filtrarAlimentos, obtenerUnidadesAlimento } = useAlimentos();
  ...
  const { stockInfo, loadingState: inventoryLoadingState, errorMessage: inventoryErrorMessage, checkStock, clearStock, isStockSufficient, getUnitCompatibilityMessage, getStockMessage } =
    useInventoryStock();
  ...
  const getUnidadesDisponibles = () => {
    ...
    return [...unidadesDisponibles, ...unidadesStock.filter((unidadStock) => !unidadesDisponibles.some((unidad) => unidad.id === unidadStock.id))];
  };

  const unidadSeleccionadaActual = getUnidadesDisponibles().find((unidad) => unidad.id === parseInt(unidadId));
  ...
  const stockBloqueaEnvio = Boolean(
    cantidadSolicitadaActual > 0 && stockInfo?.producto_encontrado && (!isStockSufficient(cantidadSolicitadaActual, simboloUnidadSeleccionada) || unitCompatibilityMessage),
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!userData || !tipoAlimento || !cantidad || cantidadNum <= 0 || !unidadId) {
    ...
    if (stockInfo && stockInfo.producto_encontrado && !isStockSufficient(cantidadNum, simboloUnidadParaSolicitud)) {
      ...
      setMensaje(unitCompatibilityMessage ? mensajeStock : `${MESSAGES.SOLICITUD.STOCK_INSUFFICIENT} ${mensajeStock}`);
      setLoading(false);
      return;
    }
  ...
  return (
    <DashboardLayout requiredRole="SOLICITANTE" title="Solicitar Alimentos" description="Rellena el formulario para enviar tu solicitud al Banco de Alimentos.">
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.7fr)]">
        <section className="space-y-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6" aria-labelledby="solicitud-form-title">
          {!ubicacion && <Alert tipo="warning" mensaje="Ubicación requerida: permite el acceso a tu ubicación en el navegador para continuar." />}
          {mensaje && <Alert tipo={mensaje.includes('éxito') ? 'success' : 'error'} mensaje={mensaje} />}
          ...
          <div className="border-b border-slate-200 pb-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 max-w-xl text-sm leading-5 text-slate-500">Especifica qué alimentos necesitas y la cantidad solicitada.</p>
              </div>
            </div>
          </div>
          ...
          {ubicacion && <UbicacionCard ubicacion={ubicacion} onUbicacionChange={manejarCambioUbicacion} />}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              ...
              <ComentariosInput comentarios={comentarios} onChange={(e) => setComentarios(e.target.value)} />
            </div>

            <div className="mt-8 flex flex-col-reverse gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end">
              <Button type="submit" disabled={loading || !ubicacion || stockBloqueaEnvio} loading={loading} accent="solicitante" className="w-full sm:w-auto">
                {loading ? (
                  'Enviando Solicitud...'
                ) : !ubicacion ? (
                  <>
        ...
        <aside className="h-fit space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-6" aria-labelledby="solicitud-context-title">
          <div className="mb-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-5 text-slate-500">Completa la información para que el Banco de Alimentos pueda revisar tu pedido.</p>
            </div>
          </div>

          <ol className="space-y-4">
            {['Selecciona el alimento que necesitas.', 'Indica la cantidad y unidad de medida.', 'Comparte tu ubicación para coordinar la entrega.'].map((step, index) => (
              <li key={step} className="flex items-start gap-3 text-sm leading-5 text-slate-500">
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-xs font-bold text-blue-700">{index + 1}</span>
                {step}
              </li>
            ))}
          </ol>
          ...
          <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm leading-5 text-blue-800">
            <p className="flex items-start gap-2">
              ...
              Puedes revisar el estado de tu solicitud desde &ldquo;Mis solicitudes&rdquo;.
            </p>
          </div>
        </aside>
      </div>
