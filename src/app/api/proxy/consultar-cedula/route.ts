...
import { enforceDocumentLookupRateLimit, resolveServerServiceUrl } from '@/lib/document-lookup-security';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!identificacion) {
      return NextResponse.json({ error: 'Parámetro "identificacion" es requerido' }, { status: 400 });
    }
    ...
    if (!/^\d{10}$/.test(identificacionLimpia) || !validarCedulaEcuatoriana(identificacionLimpia)) {
      return NextResponse.json({ error: 'Formato de identificación inválido' }, { status: 400 });
    }
    ...
    const rateLimit = await enforceDocumentLookupRateLimit({ userId: auth.profile.id, endpoint: 'consultar-cedula', documentValue: identificacionLimpia });
    ...
    return NextResponse.json({ error: 'Error al consultar identificación' }, { status: 500 });
  }
}
