...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!ruc) {
      return NextResponse.json({ error: 'Parámetro "ruc" es requerido' }, { status: 400 });
    }

    const rucLimpio = ruc.trim();

    if (!/^\d{13}$/.test(rucLimpio) || !validarRucEcuatoriano(rucLimpio)) {
      return NextResponse.json({ error: 'Formato de RUC inválido' }, { status: 400 });
    }
    ...
    const rateLimit = await enforceDocumentLookupRateLimit({ userId: auth.profile.id, endpoint: 'consultar-ruc', documentValue: rucLimpio });
