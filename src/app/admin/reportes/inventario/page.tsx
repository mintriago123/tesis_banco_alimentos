...
export default function InventoryReportPage() {
  ...
  const { processingId, updateCantidad } = useInventoryActions();

  const isLoading = loadingState === 'loading';
  const hasError = loadingState === 'error';

  const handleUpdateCantidad = useCallback(
    async (item: InventarioItem, nuevaCantidad: number) => {
  ...
  return (
    <DashboardLayout requiredRole="ADMINISTRADOR" title="Gestión de Inventario" description="Control y ajustes del stock disponible">
      <div className="p-6 space-y-6">
            ...
            <nav className="flex space-x-8 px-6" aria-label="Tabs">
              <button
                ...
                className={`flex items-center space-x-2 py-4 px-1 border-b-2 font-medium text-sm transition-colors ${
                  activeTab === 'inventario' ? 'border-red-500 text-red-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center space-x-2 py-4 px-1 border-b-2 font-medium text-sm transition-colors ${
                  activeTab === 'vencimientos' ? 'border-red-500 text-red-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                }`}
              >
        ...
        {activeTab === 'inventario' && (
          <>
            {hasError && <InventoryErrorState message={errorMessage ?? messages.loadError} onRetry={refetch} />}
            ...
            <InventoryDepositSummary depositos={depositos} inventario={inventario} />
          </>
        )}

        {activeTab === 'vencimientos' && <AlertasVencimiento diasUmbral={7} />}
      </div>
      ...
      <InventoryDetailModal item={selectedItem} isOpen={isDetailModalOpen} onClose={handleCloseModal} />
