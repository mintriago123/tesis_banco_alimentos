...
import { listarSolicitudesAltaAction, type SolicitudAltaAlimento } from '@/modules/catalogo-solicitudes';
...
import { verifyCurrentPasswordAction } from '@/modules/admin/catalogo/actions';
...
export default function AdminCatalogPage() {
  ...
  const loadSolicitudes = useCallback(async () => {
    ...
    const result = await listarSolicitudesAltaAction();
  ...
  const handleDelete = async (cascade: boolean, password?: string) => {
    ...
    if (cascade) {
      ...
      const verify = await verifyCurrentPasswordAction(password);
      if (!verify.success) {
        showError(verify.error ?? 'Contraseña incorrecta. Verifica e intenta nuevamente.');
        return;
      }
    }
  ...
  return (
    <DashboardLayout requiredRole="ADMINISTRADOR" title="Catálogo de alimentos" description="Gestiona los productos disponibles para donaciones y entregas">
      <div className="p-6 space-y-6">
        ...
        <div className="inline-flex rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${activeView === 'catalogo' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}
          >
            Catálogo
          </button>
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-semibold transition-colors ${activeView === 'solicitudes' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}
          >
            Solicitudes de alta
          </button>
        </div>

        {error && <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-600">{error}</div>}
        ...
        {activeView === 'solicitudes' ? (
          ...
          <>
            <CatalogFilters filters={filters} categories={categories} onSearchChange={setSearch} onCategoryChange={setCategory} onReset={resetFilters} />
            ...
            {loading && !filteredFoods.length ? (
              ...
              <>
                <CatalogTable foods={filteredFoods} onEdit={openEditModal} onDelete={openDeleteModal} />
                <CategoriesSection categories={categoriesWithCount} onDeleteCategory={openCategoryDeleteModal} />
              </>
            )}
          </>
        )}
      ...
      <DeleteConfirmModal open={deleteOpen} food={selectedFood} usageInfo={usageInfo} onClose={() => setDeleteOpen(false)} onConfirm={handleDelete} />
      ...
      <div className="fixed top-4 right-4 z-50 space-y-2">
        {toasts.map((toast) => {
          ...
          return (
            <div key={toast.id} className={`rounded-xl border px-4 py-3 text-sm shadow-sm ${toastClass}`}>
