...
import { buscarComprobantePorCodigoAction, marcarDonacionAprobadaAction, marcarSolicitudEntregadaAction, type SearchResult } from '@/modules/operador/validar-comprobante/actions';
...
export default function ValidarComprobantePage() {
  ...
  const buscarPorCodigo = useCallback(async () => {
    ...
    try {
      const response = await buscarComprobantePorCodigoAction(codigo);
  ...
  const handleMarcarEntregada = async () => {
    ...
    try {
      if (result.tipo === 'solicitud') {
        ...
        const response = await marcarSolicitudEntregadaAction(String(result.id), codigoVerificacion);

        if (!response.success) throw new Error(response.error);
        setResult({ ...result, estado: 'entregada' });
      } else {
        const response = await marcarDonacionAprobadaAction(result.id);
