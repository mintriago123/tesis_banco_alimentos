...
export default function OperadorSolicitudesPage() {
  ...
  const { processingId, updateEstado, procesarDonacion } = useSolicitudActions();
  ...
  const handleOpenModal = useCallback(
    async (solicitud: Solicitud, abrirModoDonacion = false) => {
      setSolicitudSeleccionada(solicitud);
      setComentarioAdmin(solicitud.comentario_admin ?? '');
      setMotivoRechazo('');
  ...
  const handleModalAprobar = useCallback(async () => {
    ...
    closeModal();
    await refetch();
  }, [solicitudSeleccionada, depositoSeleccionado, inventario, cantidadAprobar, procesarDonacion, comentarioAdmin, showError, showWarning, showSuccess, closeModal, refetch]);
  ...
  const handleDonacion = useCallback(
    async (cantidad: number, porcentaje: number, comentario: string) => {
      if (!solicitudSeleccionada) return;
      ...
      const result = await procesarDonacion(solicitudSeleccionada, cantidad, porcentaje, comentario, depositoSeleccionado);
    ...
    [solicitudSeleccionada, procesarDonacion, showError, showSuccess, showWarning, closeModal, refetch, confirm, depositoSeleccionado],
  );
  ...
  return (
    <DashboardLayout requiredRole="OPERADOR" title="Gestión de Solicitudes" description="Aprobar y rechazar solicitudes de alimentos">
      <div className="p-6 space-y-6">
        ...
        {hasError && errorMessage && <ErrorState message={errorMessage} onRetry={refetch} />}

        {tableContent}

        {hasActiveFilters && (
          <button type="button" onClick={handleResetFilters} className="text-sm text-orange-600 hover:text-orange-700 underline">
            Limpiar filtros
          </button>
        )}
        ...
        <div className="rounded-lg bg-orange-50 border border-orange-200 p-4">
          <div className="flex items-start">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-orange-800">Nota para Operadores</h3>
              <p className="mt-1 text-sm text-orange-700">
                Como operador, puedes aprobar y rechazar solicitudes, pero <strong>NO puedes revertir</strong> solicitudes ya procesadas. Para revertir una solicitud, contacta a un administrador.
              </p>
            </div>
          </div>
        </div>
