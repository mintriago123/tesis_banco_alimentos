...
export default function ComprobantePage({ params }: { params: Promise<{ codigo: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Reuses the already-authorized comprobante endpoint (handles both
        // legible codes and QR payloads, and enforces who may view what)
        // instead of querying the database directly — there's no more
        // browser-to-DB path to do that with.
        const response = await fetch(`/api/comprobante/${encodeURIComponent(codigo)}`);
        const body = await response.json();

        if (!response.ok || !body.success) {
          throw new Error(body.error ?? 'Comprobante no encontrado');
        }

        const comprobante = body.comprobante as {
          codigoComprobante: string;
          pedido: { tipo: 'solicitud' | 'donacion'; tipoAlimento: string; cantidad: number; unidad: string; estado: string; fechaCreacion: string; fechaAprobacion?: string; comentarioAdmin?: string };
          usuario: { nombre: string; email: string; telefono?: string; direccion?: string; documento?: string };
        };

        setData({
          tipo: comprobante.pedido.tipo,
          codigo: comprobante.codigoComprobante,
          estado: comprobante.pedido.estado,
          producto: comprobante.pedido.tipoAlimento,
          cantidad: comprobante.pedido.cantidad,
          unidad: comprobante.pedido.unidad,
          fechaCreacion: comprobante.pedido.fechaCreacion,
          fechaRespuesta: comprobante.pedido.fechaAprobacion ?? null,
          usuario: {
            nombre: comprobante.usuario.nombre,
            cedula: comprobante.usuario.documento ?? 'N/A',
            telefono: comprobante.usuario.telefono ?? 'N/A',
            email: comprobante.usuario.email,
            direccion: comprobante.usuario.direccion ?? 'N/A',
          },
          comentario: comprobante.pedido.comentarioAdmin,
        });
