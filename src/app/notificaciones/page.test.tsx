import { cleanup, render, screen, waitFor } from '@testing-library/react';
import userEvent from '@testing-library/user-event';
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';
import NotificacionesPage from './page';

const mocks = vi.hoisted(() => ({
  push: vi.fn(),
  useNotificaciones: vi.fn(),
  marcarComoLeida: vi.fn(),
}));

vi.mock('next/navigation', () => ({
  useRouter: () => ({ push: mocks.push }),
}));

vi.mock('@/app/components/DashboardLayout', () => ({
  default: ({ children }: { children: React.ReactNode }) => <div>{children}</div>,
}));

vi.mock('@/app/components/SupabaseProvider', () => ({
  useSupabase: () => ({ supabase: {}, user: { id: 'user-1' } }),
}));

vi.mock('@/app/components', () => ({
  LoadingSpinner: () => <div>Cargando</div>,
}));

vi.mock('@/modules/shared', () => ({
  useNotificaciones: mocks.useNotificaciones,
}));

vi.mock('@/modules/shared/components/notificaciones', () => ({
  NotificacionCard: ({
    notificacion,
    onClick,
  }: {
    notificacion: {
      id: string;
      titulo: string;
      leida: boolean;
      url_accion?: string;
    };
    onClick: (notificacion: {
      id: string;
      titulo: string;
      leida: boolean;
      url_accion?: string;
    }) => void;
  }) => (
    <button type="button" onClick={() => onClick(notificacion)}>
      {notificacion.titulo}
    </button>
  ),
  EstadisticasNotificaciones: () => <div />,
  FiltrosNotificaciones: () => <div />,
  AccionesNotificaciones: () => <div />,
  EmptyStateNotificaciones: () => <div />,
}));

const createNotification = (overrides: Record<string, unknown>) => ({
  id: 'notification-1',
  titulo: 'Notificacion',
  mensaje: 'Mensaje',
  tipo: 'info',
  categoria: 'sistema',
  metadatos: {},
  fecha_creacion: '2026-07-19T00:00:00.000Z',
  leida: false,
  ...overrides,
});

const mockNotifications = (notificaciones: ReturnType<typeof createNotification>[]) => {
  mocks.useNotificaciones.mockReturnValue({
    notificaciones,
    loading: false,
    conteoNoLeidas: notificaciones.filter((notificacion) => !notificacion.leida).length,
    marcarComoLeida: mocks.marcarComoLeida,
    marcarTodasComoLeidas: vi.fn(),
    eliminarNotificacion: vi.fn(),
  });
};

beforeEach(() => {
  mocks.push.mockReset();
  mocks.useNotificaciones.mockReset();
  mocks.marcarComoLeida.mockReset();
  mocks.marcarComoLeida.mockResolvedValue(true);
});

afterEach(() => {
  cleanup();
});

describe('NotificacionesPage', () => {
  it('navigates when a notification has a safe action URL', async () => {
    mockNotifications([
      createNotification({
        titulo: 'Ruta segura',
        url_accion: '/admin/dashboard',
      }),
    ]);

    render(<NotificacionesPage />);
    await userEvent.click(screen.getByRole('button', { name: 'Ruta segura' }));

    await waitFor(() => {
      expect(mocks.marcarComoLeida).toHaveBeenCalledWith('notification-1');
      expect(mocks.push).toHaveBeenCalledWith('/admin/dashboard');
    });
  });

  it('marks unsafe notification URLs as read without navigating', async () => {
    mockNotifications([
      createNotification({
        titulo: 'Ruta insegura',
        url_accion: 'javascript:alert(1)',
      }),
    ]);

    render(<NotificacionesPage />);
    await userEvent.click(screen.getByRole('button', { name: 'Ruta insegura' }));

    await waitFor(() => {
      expect(mocks.marcarComoLeida).toHaveBeenCalledWith('notification-1');
    });
    expect(mocks.push).not.toHaveBeenCalled();
  });
});
