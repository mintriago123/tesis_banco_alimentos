...
import {
  ...
  DonationSummary,
} from '@/modules/donante/nueva-donacion/components';
import {
  useProductSelector,
  useCatalogData,
  useUserProfile,
...
} from '@/modules/donante/nueva-donacion/hooks';
...
import { useDonanteBodegas } from '@/modules/donante/bodegas';
...
export default function NuevaDonacionPage() {
  ...
  const { bodegas, loadingState: bodegasLoadingState, errorMessage: bodegasError } = useDonanteBodegas(currentUser?.id ?? null);

  const { pasoActual, siguientePaso: avanzarPaso, pasoAnterior, resetearPaso } = useMultiStepForm(3);

  const { alimentos, unidades, cargandoAlimentos, cargandoUnidades, categoriasUnicas, obtenerUnidadesAlimento } = useCatalogData(authLoading);

  const { userProfile } = useUserProfile(currentUser, authLoading);

  const { enviando, mensaje, enviarDonacion, limpiarMensaje } = useNuevaDonacionSubmit(currentUser);
  ...
  const {
    ...
    alimentos,
    (id: string) => {
      setFormulario((prev) => ({ ...prev, tipo_producto: id, unidad_id: '' }));
    },
    limpiarMensaje,
  );
  ...
  useEffect(() => {
    if (userProfile?.direccion && !formulario.direccion_entrega) {
      setFormulario((prev) => ({ ...prev, direccion_entrega: userProfile.direccion ?? '' }));
    }
  ...
  const getUnidadesDisponibles = () => {
    ...
    return unidadesAlimento.map((u) => ({ id: u.unidad_id, nombre: u.nombre, simbolo: u.simbolo }));
  };
  ...
  const validarPaso = (paso: number): boolean => {
    switch (paso) {
      case 1: {
        ...
        const cantidadUnidad = validarCantidadParaUnidad(parseFloat(formulario.cantidad), getUnidadSeleccionada() ?? {});
  ...
  const manejarEnvio = async (e: React.FormEvent) => {
    ...
    const exito = await enviarDonacion(formulario, impacto, productoInfo, unidadInfo, alimentos);
  ...
  const renderPaso = () => {
    switch (pasoActual) {
      case 1:
        return (
          <div className="space-y-6">
            <StepHeader icon={Package} title="Información del Producto" description="Selecciona qué vas a donar" iconColor="text-emerald-600" />
            ...
            <div className="space-y-4">
              <div className="relative" onBlur={manejarBlurContainer}>
                ...
                <div className="mb-3">
                  ...
                  <p id="categoria-alimentos-ayuda" className="mt-1 text-xs text-slate-500">
                    Filtra el catálogo por tipo de alimento.
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  />
                  <p id="cantidad-ayuda" className="mt-1 text-xs text-slate-500">
                ...
                <div>
                  ...
                  <p id="unidad-ayuda" className="mt-1 text-xs text-slate-500">
                    {formulario.tipo_producto ? 'Unidades permitidas para este alimento.' : 'Selecciona primero un alimento.'}
                  </p>
                </div>
              </div>
      ...
      case 2:
        return (
          <div className="space-y-6">
            <StepHeader icon={MapPin} title="Logística de Entrega" description="Dinos cuándo y dónde podemos recoger tu donación" iconColor="text-emerald-600" />
              ...
              <div>
                ...
                {bodegasLoadingState === 'loading' || bodegasLoadingState === 'idle' ? (
                  ...
                  <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-3 text-sm text-amber-800">
                    No tienes una bodega activa.{' '}
                    <Link href="/donante/configuracion/bodegas" className="font-semibold underline">
                      Configura una bodega
                    </Link>{' '}
                    para continuar.
                  </div>
                ) : (
      ...
      case 3: {
        ...
        return (
          <div className="space-y-6">
            <StepHeader icon={Heart} title="Confirmación y Detalles Adicionales" description="Revisa tu donación y añade cualquier observación" iconColor="text-emerald-600" />
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                />
                <p id="observaciones-ayuda" className="mt-1 text-xs text-slate-500">
  ...
  return (
    <DashboardLayout requiredRole="DONANTE" title="Nueva donación" description="Registra los alimentos y coordina su llegada al Banco de Alimentos">
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.7fr)]">
        <section className="space-y-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6" aria-labelledby="current-step-title">
          {mensaje && (
            <div
              className={`rounded-xl border px-4 py-3 text-sm ${
                mensaje.includes('exitosa') ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-rose-200 bg-rose-50 text-rose-700'
              }`}
              role="status"
              aria-live="polite"
            >
          ...
          <div className="border-b border-slate-200 pb-5">
            <StepIndicator currentStep={pasoActual} totalSteps={3} stepLabels={['Producto', 'Logística', 'Confirmación']} />
          </div>

          <form onSubmit={manejarEnvio}>
            {renderPaso()}

            <StepNavigation pasoActual={pasoActual} totalPasos={3} onAnterior={pasoAnterior} onSiguiente={siguientePaso} onEnviar={manejarEnvio} enviando={enviando} />
          </form>
        </section>

        <DonationContextPanel pasoActual={pasoActual} impacto={calcularImpactoEstimado()} mostrarImpacto={!!(formulario.cantidad && formulario.unidad_id)} />
      </div>
    </DashboardLayout>
  );
}
