...
import {
  ...
  fetchCategoriasYUnidadesAction,
  listarSolicitudesAltaAction,
  type CategoriaUnidadBasica,
  type SolicitudAltaAlimento,
} from '@/modules/catalogo-solicitudes';
...
export default function SolicitarAlimentoPage() {
  ...
  const [unidades, setUnidades] = useState<CategoriaUnidadBasica[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    const [{ categorias: cats, unidades: units }, solicitudesResult] = await Promise.all([fetchCategoriasYUnidadesAction(), listarSolicitudesAltaAction()]);
  ...
  const unidadesPorTipo = unidades.reduce<Record<number, CategoriaUnidadBasica[]>>((acc, unidad) => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setSubmitting(true);
    const result = await crearSolicitudAltaAction({
      nombre,
    ...
    if (result.success && result.data?.id) {
      await sendNotification({ event: 'catalog_food_request_created', entityId: result.data.id });
  ...
  return (
    <DashboardLayout requiredRole="DONANTE" title="Solicitar alta de alimento" description="Propón alimentos para que un administrador los revise y agregue al catálogo">
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.8fr)]">
        <form onSubmit={handleSubmit} className="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {categoria === 'personalizada' && (
            <div>
              <label htmlFor="categoriaPersonalizada" className="mb-1 block text-sm font-medium text-slate-700">
                          ...
                          <div key={unidad.id} className="relative">
                            <button
                              ...
                              className={`w-full rounded-lg border px-3 py-2 text-left text-sm transition-colors ${
                                selected ? 'border-emerald-400 bg-emerald-50 text-emerald-900' : 'border-slate-200 bg-white text-slate-700 hover:border-emerald-300'
                              }`}
                            >
          ...
          <div className="flex justify-end">
            <Button type="submit" disabled={submitting} loading={submitting} accent="donante">
        ...
        <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {loading ? (
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">Aún no has enviado solicitudes de alta.</div>
          ) : (
            <div className="space-y-3">
              {solicitudes.map((solicitud) => (
                <article key={solicitud.id} className="rounded-xl border border-slate-200 p-4">
                  ...
                  <p className="mt-2 text-xs text-slate-500">Enviada el {new Date(solicitud.created_at).toLocaleDateString('es-EC')}</p>
                  {solicitud.comentario_admin && <p className="mt-2 rounded-lg bg-slate-50 p-2 text-xs text-slate-600">{solicitud.comentario_admin}</p>}
                </article>
              ))}
            </div>
          )}
