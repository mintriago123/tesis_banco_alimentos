...
import { useRegister, RoleSelector, AuthInput, PasswordInput, AuthButton, AuthMessage, AUTH_CONSTANTS, type Rol } from '@/modules/auth';
...
export default function PaginaRegistroSimple() {
  ...
  const [datos, setDatos] = useState({ email: '', password: '', confirmPassword: '' });
  ...
  const manejarEnvio = async (e: React.FormEvent) => {
    ...
    await registrar({ ...datos, rol: rolSeleccionado });
  };
  ...
  if (registroExitoso) {
    return (
      <div className="text-center space-y-4">
        ...
        <AuthButton onClick={irALogin} textoNormal="Ir al inicio de sesión" variant="primary" />
      </div>
    );
  }
  ...
  return (
    <form className="space-y-4 max-w-md mx-auto" onSubmit={manejarEnvio}>
      ...
      <RoleSelector rolSeleccionado={rolSeleccionado} onSeleccionarRol={setRolSeleccionado} />
      ...
      {rolSeleccionado && (
        <>
          ...
          <AuthButton type="submit" disabled={estaCargando} cargando={estaCargando} textoNormal="Crear cuenta" textoCargando="Creando..." />
      ...
      <div className="text-center text-sm text-gray-600 mt-4">
        ...
        <Link href={AUTH_CONSTANTS.RUTAS.INICIAR_SESION} className="font-medium text-blue-600 hover:text-blue-500">
          Inicia sesión aquí
        </Link>
      </div>
    </form>
